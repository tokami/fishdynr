  ...
  iteration = 1)
{
  ...
  for (iter in iteration) {
    ...
    ages <- 0:(floor(max(inds$A)))
    
    ## Pmega (Lopt + 10%) (>.3)
    Lts <- HCR.VBGF(Linf.mu, K.mu, t0, ages)
    Wts <- HCR.LtWt.fit(LWa, LWb, Lts)
    stable.biomass <- (1*exp(-M*ages))*Wts
    Lopt <- Lts[stable.biomass == max(stable.biomass)]
    Lopt0.9 <- Lopt * 0.9
    Lopt1.1 <- round(1.1 * Lopt, 1)
    Lmat_ind <- midLengths >= Lmat.f
    Lopt_ind <- midLengths > Lopt0.9 & midLengths < Lopt1.1
    Lmega_ind <- midLengths >= Lopt1.1
    Pmat <- vector('numeric', length(dates))
    for(i in 1:length(dates)){
      Pmat[i] <- sum(c_list[[i]][Lmat_ind], na.rm = T) / sum(c_list[[i]], na.rm = T)
    }
    Popt <- vector('numeric', length(dates))
    for(i in 1:length(dates)){
      Popt[i] <- sum(c_list[[i]][Lopt_ind], na.rm = T) / sum(c_list[[i]], na.rm = T)
    }
    Pmega <- vector('numeric', length(dates))
    for(i in 1:length(dates)){
      Pmega[i] <- sum(c_list[[i]][Lmega_ind], na.rm = TRUE) / 
        sum(c_list[[i]], na.rm = TRUE)
    }
    Pobj <- vector('numeric', length(dates))
    for(i in 1:length(dates)){
      Pobj[i] <- Pmat[i] + Popt[i] + Pmega[i]
    }
